...
def _symbol_already_open(symbol: str) -> bool:
    """
    Prevent opening multiple simultaneous positions in the same symbol.
    """
...
def _calculate_volume_from_risk(symbol: str, entry_price: float, stop_loss: float) -> float | None:
    """
    ...
    This is a practical thesis-friendly implementation, but you should verify
    the contract-size interpretation for your Pepperstone symbols.
    """
    ...
    # Make sure we do not fall below min after rounding
    if volume < volume_min:
        volume = volume_min

    # Clean float precision a bit
...
def execute_best_trade(account: dict, ranked_opportunities: list[dict]) -> None:
    ...
    # Natural open-trade cap from 2h / 15min = 8
    ...
    # Optional but very useful: only one open trade per symbol
